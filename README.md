...
- ⭐ `PrintLayoutPart`:
  - `fonts` (`PrintFont[]`):
    ...
    - `note` (`string`)
  - `fontUsages` (`PrintFontUsage[]`):
    - `id`\* (`string`, one of the above font EIDs).
    - `section`\* (`string[]` 📚 `print-layout-sections`, fieldset: title, body, comment, proem, other): the section(s) where the font is used.
